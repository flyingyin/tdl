...
func NewDownload() *cobra.Command {
	...
	cmd.Flags().StringVarP(&opts.Dir, dir, "d", ".", "specify the download directory. If the directory does not exist, it will be created automatically")
	...
	cmd.Flags().BoolVar(&opts.SkipSame, "skip-same", true, "skip files with the same name(without extension) and size")
	...
	cmd.Flags().BoolVar(&opts.Group, "group", true, "auto detect grouped message and download all of them")
